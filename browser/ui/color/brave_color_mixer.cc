...
SkColor GetColorContrastingToToolbar(const ui::ColorMixer& mixer,
                                     const ColorPropertiesMapEntry& entry,
                                     ProfileType profile_type) {
  const auto toolbar_color = mixer.GetResultColor(kColorToolbar);
      ...
      switch (entry.property_id) {
        ...
        case BraveThemeProperties::
          ...
          mixer[entry.color_id] = {
              GetBraveThemeColor(key, entry, profile_type)};
          break;
        }
#if BUILDFLAG(ENABLE_SIDEBAR)
        ...
        case BraveThemeProperties::COLOR_SIDEBAR_ITEM_DRAG_INDICATOR_COLOR: {
          mixer[entry.color_id] = {
              GetColorContrastingToToolbar(mixer, entry, profile_type)};
          break;
        }
